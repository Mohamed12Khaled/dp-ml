...
def train_and_evaluate(df):
    ...
    # Evaluation
    results = {}
    results['Random Forest R2'] = r2_score(y_test, y_pred_rf)
    results['Gradient Boosting R2'] = r2_score(y_test, y_pred_gbr)
    results['Logistic Regression Accuracy'] = accuracy_score(y_test, y_pred_logistic)
    results['Logistic Regression Confusion Matrix'] = confusion_matrix(y_test, y_pred_logistic)
    results['Logistic Regression Classification Report'] = classification_report(y_test, y_pred_logistic)
    results['XGBoost MAE'] = mean_absolute_error(y_test, y_pred_xgb)
    results['XGBoost MSE'] = mean_squared_error(y_test, y_pred_xgb)
    results['XGBoost R2'] = r2_score(y_test, y_pred_xgb)
